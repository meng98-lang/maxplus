...
import { Shield, Zap, Leaf, Truck, Clock, CheckCircle, Star, ArrowRight } from 'lucide-react';
...
export default async function Home() {
  ...
  const product = products[0]; // Only one product
    ...
    <>
      ...
      <section className="relative min-h-screen flex items-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-20">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_50%,_rgba(251,191,36,0.15),_transparent_50%)]" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 py-20 grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            {/* Badge */}
            <div className="inline-flex items-center gap-2 mb-6 px-4 py-2 bg-amber-500/10 border border-amber-500/20 rounded-full backdrop-blur-sm">
            ...
            <p className="text-lg text-gray-400 mb-8 leading-relaxed">
              Japanese-engineered topical formula for enhanced stamina, confidence, and intimate wellness. Developed by Kawai Pharmaceutical with traditional herbal extracts.
            </p>
            ...
            <div className="flex items-center gap-3 mb-8">
              <div className="flex items-center">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className={`w-5 h-5 ${i < Math.floor(product?.rating || 5) ? 'text-amber-400 fill-amber-400' : 'text-gray-600 fill-gray-600'}`} />
                ))}
              </div>
              <span className="text-gray-400">{product?.rating || 4.9} ({product?.reviews || 857} reviews)</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-12">
              <a 
                href="#packages" 
                className="px-10 py-4 bg-amber-500 hover:bg-amber-600 text-black font-semibold rounded-lg text-lg transition-all transform hover:scale-105 shadow-lg shadow-amber-500/20 text-center"
              >
                Choose Package
              </a>
              <a 
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={i} className="flex items-center gap-3 text-gray-400 bg-slate-800/50 rounded-lg px-4 py-3 border border-slate-700">
          ...
          <div className="relative">
            <div className="aspect-square bg-gradient-to-br from-slate-700 to-slate-800 rounded-3xl p-12 border border-slate-600 shadow-2xl">
              {product?.imageUrl ? (
                <img 
                ...
                <div className="w-full h-full flex items-center justify-center text-8xl text-slate-600">📦</div>
              )}
            </div>
            {/* Price Tag */}
            <div className="absolute -bottom-6 -right-6 bg-amber-500 text-black rounded-2xl p-6 shadow-2xl">
              <div className="text-sm font-medium opacity-75">Starting from</div>
              <div className="text-4xl font-bold">$49.9</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Packages Section */}
      <section id="packages" className="py-24 bg-gradient-to-b from-slate-900 to-slate-800">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { 
                ...
                comparePrice: 69.9,
                badge: 'Popular',
                highlights: ['30-day supply', 'Extend intimate time', 'Fast-acting formula'],
                color: 'border-slate-700'
              },
              { 
                name: '2 Boxes', 
                subtitle: 'Treatment Pack',
                price: 89.9, 
                comparePrice: 139.8,
                badge: 'Best Value',
                highlights: ['60-day supply', 'Treat ED & premature', 'Save $49.90'],
              ...
              { 
                ...
                comparePrice: 209.7,
                badge: 'Best Seller',
                highlights: ['90-day supply', 'Maximum results', 'Save $89.80'],
                color: 'border-slate-700'
              },
            ].map((pkg, i) => (
              <div key={i} className={`bg-slate-800 border ${pkg.color} rounded-2xl overflow-hidden hover:scale-105 transition-all flex flex-col`}>
                {/* Badge */}
                ...
                <div className="p-8 flex flex-col flex-1">
                  <div className="text-amber-400 text-sm font-semibold mb-2 uppercase tracking-wider">{pkg.subtitle}</div>
